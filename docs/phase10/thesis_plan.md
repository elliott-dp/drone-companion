...
|---|---|---|---|---|
| Cardiac budget closes, worst-realistic | 0.095 vs 0.110 rad synthetic; 0.033–0.049 on measured hover (2.2–3.3× margin) | [meas-sim, measured motion input] | validation D + exp10 | bench V2; project-airframe ULogs |
| RBEC dwell envelope: altitude-hold drift ≲1 cm/s (measured requirement) | 0.11 cm/s → 3.3× inside budget; 11 cm/s → 1.5× over | [meas-sim, measured motion input] | exp10 B.3 | wire the EKF-velocity dwell gate |
...
| P5 | Hover ULogs → `hover_ingest` → exp3/exp4 on measured sway — **done at the representative tier 2026-08** (`exp10_hover_measured.py`, public hover-endurance ULog; [`radar_rbec_validation_exp10.md`](radar_rbec_validation_exp10.md)). Final tier: fly HOVER_CAPTURE.md on the project airframe and re-run unchanged | Ch5 | rerun: minutes | project-airframe flights (final tier only) | Budget margin grows 1.16× → 2.2–3.3× on measured hover; new measured envelope requirement: altitude-hold drift ≲1 cm/s per dwell (11 cm/s busts the budget 1.5×); seam cliff proven motion-invariant |
